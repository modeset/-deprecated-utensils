...
describe 'Triggerable', ->
  ...
  beforeEach ->
    ...
    @defaulter = new utensil.Triggerable(@defaulter_el)
    @hoverer = new utensil.Triggerable(@hover_el)
    @focuser = new utensil.Triggerable(@focus_el)
    @delayer = new utensil.Triggerable(@delay_el)
    @radio = new utensil.Triggerable(@radios_el)
    @check = new utensil.Triggerable(@checks_el)
  ...
  describe '#dispose', ->
    ...
    it 'does not call removeListeners when event type is manual since no listeners are added', ->
      spyme = new utensil.Triggerable(@dom, trigger:'manual')
